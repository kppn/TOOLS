...
static int setroute(char * dev, char * route, char * user_addr, sa_family_t family) 
{
	char buf[1024];
	int ret;
	char * family_opt;
	...
	sprintf(buf, "/sbin/ip %s route add %s dev %s table 1", family_opt, route, dev);
	DEBUGPRINT(1, ("set route : %s\n", buf));
	ret = system(buf);

	sprintf(buf, "/sbin/ip %s rule add to %s table 1", family_opt, route);
	DEBUGPRINT(1, ("set rule : %s\n", buf));
	ret = system(buf);
...
int setif(Session * sp, char * own_gtpu_addr) 
{
	int ret = 0;
	...
	if (sp->dst_route4[0] != '\0')
		ret += setroute(sp->tun_dev, sp->dst_route4, sp->user_addr4, AF_INET);
	if (sp->dst_route6[0] != '\0')
		ret += setroute(sp->tun_dev, sp->dst_route6, sp->user_addr6, AF_INET6);
...
static int unsetrule(char * user_addr, sa_family_t family)
{
	char buf[1024];
	int ret;
	char * family_opt;
	...
	sprintf(buf, "/sbin/ip %s rule del from %s table 1", family_opt, user_addr);
	DEBUGPRINT(1, ("set rule : %s\n", buf));
	ret = system(buf);
...
int unsetif(Session * sp, char * own_gtpu_addr)
{
	int ret = 0;

	if (sp->user_addr4[0] != '\0')
		ret += unsetrule(sp->user_addr4, AF_INET);
	if (sp->user_addr6[0] != '\0')
		ret += unsetrule(sp->user_addr6, AF_INET6);
