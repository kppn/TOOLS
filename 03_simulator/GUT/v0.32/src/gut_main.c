...
/* This function is garbedge of old version */
#if 0
			...
			uint8_t req_seq[2], struct environ * env)
{
	...
	if( sendto(udpfd, gtp_p, gtp_header_len + packsize, 0, 
	    (struct sockaddr *)udp_dst, sizeof(*udp_dst)) < 0) {
		printf("udp socket send error\n");
		return -1;
	}
...
/*======================================================================================*/
int packet_transfer(int tunfd, int udpfd, struct sockaddr_in * udp_dst, 
		    struct environ *env)
{
	uchar buf[BUFSIZE];
	int packsize;
	...
	while(1) {
		...
		if (FD_ISSET(udpfd, &fds)) {
			...
			if (get_gtp_msgtype(buf) == GTP_MSGTYPE_ECHO_REQ) {
				uint8_t req_seq[2];
				uint8_t dummy_recovery[2] = {0x0e, 0x00};
				...
				packsize = (packsize + 2) - gtp_header_len;
				DEBUGPRINT(1, ("send GTP Echo Response. size: %d\n", gtp_header_len));
				DEBUGPRINT(1, ("send GTP Echo response packet. size: %d\n", packsize));
				send_gtp_echo_resp(udpfd, user_p, packsize, udp_dst, req_seq, env);
				continue;
			}
			if (get_gtp_msgtype(buf) == GTP_MSGTYPE_END_MARKER) {
				printf("receive End Marker \n");
				continue;
			}
			if (get_gtp_msgtype(buf) == GTP_MSGTYPE_ERR_IND) {
...
int main(int argc, char ** argv)
{
	...
	char confname[128] = "gut.conf";

	if (argc > 1) 
		strcpy(confname, argv[1]);
	DEBUGPRINT(1, ("configu file : %s\n", confname));

	read_environ(&env, confname);
	disp_environ(env);


	/**** open tun device						****/
	/**** read from config file and set IP/Router to tun device 	****/
	tunfd = tun_sock_open(env.tun_dev);
	if (tunfd < 0) {
	...
	/* Packet Transfer */
	if (packet_transfer(tunfd, udpfd, &udp_dst, &env) < 0) {
