...
void init_tundev_pool(int tundev_env_base, int tundev_env_max);
...
int init_environ(Environ * env, char * filename) 
{
	...
	init_tundev_pool(env->base_tunnel_device, env->max_tunnel_device);
...
void disp_environ(Environ * env) 
{
	...
	printf("%-*s : %d\n",	indent, "base_tunnel_device",	env->base_tunnel_device);
...
int alloc_tundev(char * tun_dev, int tundev_env_base, int tundev_env_max)
{
	int i;
	...
	sprintf(tun_dev, "%s%d", "tun", i + tundev_env_base);
	return 0;
}



void free_tundev(char * tun_dev, int tundev_env_base, int tundev_env_max)
{
	long dev_num;

	dev_num = strtol(tun_dev+3, NULL, 10);
	if (dev_num < 0 || (tundev_env_base + tundev_env_max) <= dev_num)
		return;

	tundev_pool[dev_num-tundev_env_base] = TUNDEV_UNUSED;
}
